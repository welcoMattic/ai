 ...
 * Add `EdenAiJobClient`, resolving the jobs the asynchronous speech-to-text endpoint hands out through `Job\JobClientInterface`, so waiting for one is the caller's decision instead of a poll loop inside `invoke()`
