...
final class ResultConverterTest extends TestCase
{
    ...
    /**
     * Bounding boxes are an extra on top of the text, and their shape varies per provider: a box
     * that cannot be read is skipped instead of losing the whole (already billed) result.
     */
    public function testItSkipsBoundingBoxesItCannotRead()
    {
        $converter = new ResultConverter();
        $httpResponse = $this->createResponse([
            'status' => 'success',
            'output' => [
                ...
                'bounding_boxes' => [
                    null,
                    'not-a-box',
                    ['text' => 'Hello', 'left' => 0.1, 'top' => 0.2, 'width' => 0.3, 'height' => 0.4],
                    ['text' => ['unexpected']],
                ],
            ],
        ]);

        $ocr = $converter->convert(new RawHttpResult($httpResponse))->getContent();

        $this->assertInstanceOf(OcrResult::class, $ocr);
        $this->assertCount(2, $ocr->getBoundingBoxes());
        $this->assertSame('Hello', $ocr->getBoundingBoxes()[0]->getText());
        $this->assertSame(0.1, $ocr->getBoundingBoxes()[0]->getLeft());
        $this->assertSame('', $ocr->getBoundingBoxes()[1]->getText());
    }

    public function testItIgnoresANonListOfBoundingBoxes()
    {
        $converter = new ResultConverter();
        $httpResponse = $this->createResponse([
            'status' => 'success',
            'output' => ['text' => 'Hello world', 'bounding_boxes' => 'oops'],
        ]);
