...
final class ModelApiCatalogTest extends TestCase
{
    ...
    /**
     * A gateway that could not be reached keeps saying so: the failure is not retried on every
     * lookup, but it is not swallowed into a "model not found" on the second call either.
     */
    public function testItKeepsReportingADiscoveryFailureWithoutRetryingIt()
    {
        $httpClient = new MockHttpClient(new MockResponse('', ['http_code' => 503]));
        $catalog = new ModelApiCatalog($httpClient);
        ...
        $this->assertInstanceOf(RuntimeException::class, $first);

        try {
            $catalog->getModel('openai/gpt-4o-mini');
            $this->fail('The second lookup should have reported the discovery failure again.');
