...
final class ModelClientTest extends TestCase
{
    ...
    public function testItSendsStringPayloadAsTextForTextBasedModels()
    {
        $httpClient = new MockHttpClient(function (string $method, string $url, array $options): MockResponse {
            ...
            $this->assertSame(['text' => 'Hello world'], $body['input']);
    ...
    public function testItSendsFileToUniversalAiEndpoint()
    {
        $httpClient = new MockHttpClient(function (string $method, string $url, array $options): MockResponse {
            ...
            $this->assertSame('https://api.edenai.run/v3/universal-ai', $url);

            $body = json_decode($options['body'], true);
            $this->assertSame('ocr/ocr/google', $body['model']);
            $this->assertSame(['file' => 'https://example.com/document.pdf'], $body['input']);
    ...
    public function testItMergesOptionsIntoInput()
    {
        $httpClient = new MockHttpClient(function (string $method, string $url, array $options): MockResponse {
            $body = json_decode($options['body'], true);
            $this->assertSame('fr', $body['input']['language']);
            $this->assertSame('invoice', $body['input']['document_type']);
            $this->assertSame('https://example.com/invoice.pdf', $body['input']['file']);
    ...
    public function testItKeepsRootOptionsAtRootLevel()
    {
        $httpClient = new MockHttpClient(function (string $method, string $url, array $options): MockResponse {
            $body = json_decode($options['body'], true);
            $this->assertSame(['ocr/ocr/microsoft'], $body['fallbacks']);
            $this->assertTrue($body['show_original_response']);
            $this->assertSame(['google' => ['timeout' => 10]], $body['provider_params']);
            $this->assertArrayNotHasKey('fallbacks', $body['input']);
            $this->assertArrayNotHasKey('show_original_response', $body['input']);
            $this->assertArrayNotHasKey('provider_params', $body['input']);
    ...
    public function testItAcceptsStringPayloadAsFileReference()
    {
        $httpClient = new MockHttpClient(function (string $method, string $url, array $options): MockResponse {
            $body = json_decode($options['body'], true);
            $this->assertSame(['file' => 'a9f3c2e1-6d4b-4f4e-9d2a-1c2b3d4e5f6a'], $body['input']);
