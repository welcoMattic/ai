...
final class FileUploaderTest extends TestCase
{
    public function testItUploadsFileAndReturnsFileId()
    {
        $httpClient = new MockHttpClient(function (string $method, string $url, array $options): MockResponse {
            ...
            $this->assertSame('https://api.edenai.run/v3/upload', $url);
            $this->assertStringContainsString('multipart/form-data', implode(' ', $options['normalized_headers']['content-type'] ?? []));
    ...
    {
        $httpClient = new MockHttpClient(function (string $method, string $url, array $options): MockResponse {
            $this->assertStringContainsString('filename="file"', self::readBody($options));
    ...
    public function testItForwardsTheRetentionWhenConfigured()
    {
        $httpClient = new MockHttpClient(function (string $method, string $url, array $options): MockResponse {
            ...
            $this->assertStringContainsString('name="expires_in_days"', $body);
