 ...
 * The base64 content is exposed directly, so no extra request is needed. Its format follows the
 * requested "response_format"/provider and is read back from the extension of the resource URL,
 * which is the only place the response states it.
 *
 ...
 *
 * Note that the "output_schema" advertised by GET /v3/info/image/generation describes a single
 * item without the enclosing "items" list; the endpoint really wraps them.
 *
...
final class ResultConverter implements ResultConverterInterface
{
    ...
    public function convert(RawResultInterface $result, array $options = []): ResultInterface
    {
        ...
        foreach ($items as $item) {
            ...
            $resourceUrl = $item['image_resource_url'] ?? null;
            $resourceUrl = \is_string($resourceUrl) ? $resourceUrl : null;

            // Validates the payload and throws when it is not decodable base64.
            $images[] = BinaryResult::fromBase64($item['image'], $this->resolveMimeType($resourceUrl));
    ...
    /**
     * The response states the image format nowhere but in the extension of the resource URL, so
     * that is what is read; PNG is Eden AI's default and the fallback when there is no URL.
     */
    private function resolveMimeType(?string $imageResourceUrl): string
    {
        if (null === $imageResourceUrl) {
            return 'image/png';
        }

        $extension = strtolower(pathinfo(parse_url($imageResourceUrl, \PHP_URL_PATH) ?: '', \PATHINFO_EXTENSION));
